...
function ViewCard() {
    ...
    const isBooked = cardDetails?.isBooked;
    ...
    const handlePayment = async () => {
        try {
            ...
            const { data } = await axios.post(
                ...
                { amount: total } // dynamic total
            );
            const order = data.order;
            // 2. Razorpay options
            const options = {
                key: "rzp_test_SVXCeTKqsf3mm0", // 👈 your key
                amount: order.amount,
                currency: "INR",
                name: "RentNest",
    ...
    return (
        <div className='w-[100%] h-[100vh] bg-[white] flex items-center justify-center gap-[10px] flex-col overflow-auto  relative'>
            <div className='w-[50px] h-[50px] bg-lime-700 cursor-pointer absolute top-[5%] left-[20px] rounded-[50%] flex items-center justify-center' onClick={() => navigate("/")}><FaArrowLeftLong className='w-[25px] h-[25px] text-[white]' /></div>
            ...
            <div className='w-[95%] h-[50px] flex items-center justify-start px-[110px]'>{cardDetails.host == userData._id && <button className='px-[30px] py-[10px] bg-lime-700 text-[white] text-[18px] md:px-[100px] rounded-lg  text-nowrap' onClick={() => setUpdatePopUp(prev => !prev)}>
                Edit listing
            </button>}
                {cardDetails.host != userData._id && <button className='px-[30px] py-[10px] bg-lime-700 text-[white] text-[18px] md:px-[100px] rounded-lg   text-nowrap' onClick={() => setBookingPopUp(prev => !prev)}>
                    Reserve
                </button>}
            </div>
            ...
            {updatePopUp && <div className='w-[100%] h-[100%] flex items-center justify-center  bg-[#000000c6] absolute top-[0px] z-[100] backdrop-blur-sm'>
                ...
                <RxCross2 className='w-[30px] h-[30px] bg-lime-700 cursor-pointer absolute top-[6%] left-[25px] rounded-[50%] flex items-center justify-center' onClick={() => setUpdatePopUp(false)} />
                ...
                >
                    ...
                    <div className='w-[200px] h-[50px] text-[20px] bg-lime-700 text-[white] flex items-center justify-center rounded-[30px] absolute top-[5%] right-[10px] shadow-lg'>
                        Update your details
                    </div>
                    ...
                    <div className='w-[100%] flex items-center justify-center gap-[30px] mt-[20px]'>
                        <button className='px-[10px] py-[10px] bg-lime-700 text-[white] text-[15px] md:px-[100px] rounded-lg md:text-[18px] text-nowrap  ' onClick={handleUpdateListing} disabled={updating}>{updating ? "updating..." : "Update Listing"}</button>
            ...
            {bookingPopUp && <div className='w-[100%] min-h-[100%] flex items-center justify-center flex-col gap-[30px] bg-[#ffffffcd] absolute top-[0px] z-[100] p-[20px] backdrop-blur-sm md:flex-row md:gap-[100px]'>
                <RxCross2 className='w-[30px] h-[30px] bg-lime-700 cursor-pointer absolute top-[6%] left-[25px] rounded-[50%] flex items-center justify-center' onClick={() => setBookingPopUp(false)} />
                ...
                <form className='max-w-[450px] w-[90%] h-[450px] overflow-auto bg-[#f7fbfcfe] p-[20px] rounded-lg flex items-center justify-start flex-col gap-[10px]    border-[1px] border-[#dedddd]' onSubmit={(e) => {
                    ...
                    <div className='w-[100%] h-[70%] mt-[10px] rounded-lg p-[10px]'>
                        ...
                        <div className='w-[100%] flex items-center justify-center pt-5 '>
                            {/* <button
                                className='px-[80px] py-[10px] bg-lime-700 text-[white] text-[18px] rounded-lg'
                            ...
                            <button
                                className={`px-[80px] py-[10px] text-white text-[18px] rounded-lg 
    ${isBooked ? "bg-gray-400 cursor-not-allowed" : "bg-lime-700"}`}
                            ...
                            >
                                {isBooked ? "Already Booked" : booking ? "Booking..." : "Pay & Book"}
                            </button>
                        </div>
